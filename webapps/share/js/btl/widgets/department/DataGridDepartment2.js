define(["dojo/_base/declare",
    ...
    var transferEmployee = function( _this, employeeId, startDepartmentId, endDepartmentId ){
        var success = false;
        if (endDepartmentId == null)
            return success;
        if (startDepartmentId == endDepartmentId)
            return success;
        if (startDepartmentId == "all" && endDepartmentId == "")
            return success;
        if (startDepartmentId == "" && endDepartmentId == "all")
            return success;

        var sendData = new Object();
        if (startDepartmentId != null && startDepartmentId != "")
            sendData["assoc_btl-emp_department_removed"] = startDepartmentId;
        if (endDepartmentId != "" && endDepartmentId != "all")
            sendData["assoc_btl-emp_department_added"] = endDepartmentId;

        var employee = {};
        employee.nodeRef = employeeId;

        _this.alfLog("log", "btl_DataGridDepartment__transferEmployee", employee);
        _this.serviceXhr({
        ...
        if (success){
            ...
            var selectedTreeNode = getSelectedTreeNode(); // нужен потому что startDepartmentId содержит текущий департамент пользователя, а не выбранный
            if (selectedTreeNode.item.id != "all") // При переносе из "Все сотрудники" не должно удаляться
   ...
   return declare([DataGrid, CoreXhr, Core], {
	   ...
	   allChild: false,
	   type: "dep_children_employee",
	   parentTreeType: "btl-depart:department-folder",
	   toolbar : {
           cls: 'pq-toolbar-crud',
           items: [
	    	...
	    	this.grid.pqGrid({
	    	    ...
	    	    rowMouseDown: function( event, ui ) {
                    ...
                    window.onmouseup = function(event) {
                        ...
                        //alert("selected " + (selectedNode != null ? selectedNode.item.id : "null") + " dep_ref "+  (rowData.department_ref != null ? rowData.department_ref : "null"));

                        var success = transferEmployee(_this, employeeId, rowData.department_ref, hoverNode != null ? hoverNode.item.id : null);
                    };
                },
	    	    showTitle: false
