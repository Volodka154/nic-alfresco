 ...
 */
define(
    ["jquery", "jqueryui"], function($ ){

	var regDate = /^(0[1-9]|[12][0-9]|3[01])[-|.](0[1-9]|1[0-2])[-|.]([1-9][0-9]{3})$/; //dd-mm-yyyy || dd.mm.yyyy
...
(function($) {
	...
	fnPG._regional = {
		strDisplay: "Отображено {0} до {1} из {2} записей.",
...
(function($) {
	...
	fn._regional = {
		...
		strNothingFound: "Поиск не дал результатов",
		strPrevResult: "Назад",
		strSearch: "Поиск",
		strSelectedmatches: "Выбрано {0} из {1}"
	};
	...
	fn.remoteRequest = function(objP) {
		...
		if (typeof DM.getUrl == "function") {
			...
			if (typeof DM.url == "string") {
				url = DM.url;
				var sortQueryString = {}, filterQueryString = {}, pageQueryString = {}, groupModel = {};
				...
				if (thisOptions.groupModel)
				{					
					groupModel = {
						pq_groupModel_dataIndx: thisOptions.groupModel.dataIndx[0]
					}					
				}
				...
				}, filterQueryString, pageQueryString, sortQueryString, postData, postDataOnce, groupModel)
			}
		} if (!url) {
			return
		}
	...
	fn.refreshToolbar = function(){
		...
		if (toolbar) {
			var tb = toolbar,
				...
				attr = attr ? attr : "",
				items = tb.items;
			var $toolbar = $("<div class='" + cls + "' style='" + style + "' " + attr + " ></div>").insertBefore($(".pq-grid-footer", this.element));
			$toolbar.pqToolbar({
				items: items,
				gridInstance: this
			});
...
(function($) {
	...
	fn.filter = function(objP) {
		...
		var that = this,
			...
			that._trigger("beforeFilter",null, {
                    			dataModel: DM,
                    			filterModel: FM
                    		});
	...
	_pSort.__sortLocalData = function(sorters, data) {
		...
		function sort_date(obj1, obj2, dataIndx, dir) {
			...
			//var regDate = /^(0[1-9]|[12][0-9]|3[01])[-|.]([01][12])[-|.]([1-9][0-9]{3})$/; //dd-mm-yyyy || dd.mm.yyyy
			//var regDatePoint = /^(0[1-9]|[12][0-9]|3[01])[.]([01][12])[.]([1-9][0-9]{3})$/;//

			if(regDate.test(val1)){
				val1 = dateParse(val1) ;
			}else{
				val1 = val1 ? Date.parse(val1) : 0;
			}

			if(regDate.test(val2)){
				val2 = dateParse(val2) ;
			}else{
	...
	cFilterData.conditions = {
		...
		contain: function(cd, value) {
		    if(cd.indexOf("TYPE='DOCUMENTTYPE'") != -1){
            	var el = document.createElement('span');
                el.innerHTML = cd;
                cd = el.getElementsByClassName("REG-NUMBER-DOC")[0];
                cd = cd ? cd.innerText: "";
		...
		gte: function(cd, value) {
		    var regEx = /^([1-9]|0[1-9]|[12][0-9]|3[01])-(0[1-9]|1[0-2])-(\d{4})$/;
            if(regEx.test(cd)){
                var date1 = new Date(value.split('-')[2].toString(), (Number(value.split('-')[1]) - 1).toString(), value.split('-')[0].toString());
                var cdDate = new Date(cd.split('-')[2].toString(), (Number(cd.split('-')[1]) - 1).toString(), cd.split('-')[0].toString());
                if (cdDate >= date1) {
                    return true
                }
            }else{
		...
		between: function(cd, value, value2) {
		    ...
		    if(regEx.test(cd)){
                var date1 = new Date(value.split('-')[2].toString(), (Number(value.split('-')[1]) - 1).toString(), value.split('-')[0].toString());
                var date2 = new Date(value2.split('-')[2].toString(), (Number(value2.split('-')[1]) - 1).toString(), value2.split('-')[0].toString());
                var cdDate = new Date(cd.split('-')[2].toString(), (Number(cd.split('-')[1]) - 1).toString(), cd.split('-')[0].toString());
                if (cdDate && cdDate >= date1 && cdDate <= date2) {
                    return true
                }
		    }else{
                if (cd && cd >= value && cd <= value2) {
                    return true
                }
            }
		},
