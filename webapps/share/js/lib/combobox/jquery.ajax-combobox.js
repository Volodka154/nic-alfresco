    ...
    ["jquery"], function ($) {
        (function ($) {
            ...
            $.fn.ajaxComboBox = function (source, option) {
                ...
                return (option != undefined && option.instance != undefined && option.instance) ? {
                    "this": this,
                    "op": $(arr)
                } : this;
            };
            ...
            function AjaxComboBox(combo_input, source, option) {
                ...
                var _this = this;
                YAHOO.Bubbling.on('changeAjaxComboBoxDataUrl', function (layer, args) {
                    _this._changeAjaxComboBoxDataUrl(_this, layer, args);
                });
                YAHOO.Bubbling.on('clearAjaxComboBox', function (layer, args) {
                    _this._clearAjaxComboBox(_this, layer, args);
                });
            }
            ...
            $.extend(AjaxComboBox.prototype, /** @lends AjaxComboBox.prototype */ {
                ...
                _setOption1st: function (source, option) {
                    return $.extend({
                        ...
                        group_img: '/share/res/js/lib/combobox/user-group-icon.png',
                        bind_to: false,
                        navi_simple: false,
                        ...
                        withButton: true,
                        navi_prevNext: false,
                        navi_simpleFirstLast: true,
                        withSubInfoElem: false,
                        hasAreaMaxHeight: false,
                        changeNoResultText: true,
                        noResultText: "Поиск не дал результата",
                    ...
                    switch (this.option.lang) {
                        ...
                        case 'en':
                            message = {
                                ...
                                get_all_btn: 'Все (Down key)',
                                get_all_alt: '(button)',
                                close_btn: 'Закрыть (Tab key)',
                                close_alt: '(button)',
                                loading: 'Загрузка...',
                                loading_alt: '(loading)',
                                page_info: 'num_page_top - num_page_end из cnt_whole',
                                select_ng: 'Внимание : Пожалуйста выбарете из списка.',
                                select_ok: 'OK ',
                ...
                _changeAjaxComboBoxDataUrl: function (self, layer, args) {

                    var obj = args[1];
                    if (obj.fieldHtmlId != self.option.fieldHtmlId) {
                        return;
                    }

                    self.option.source = obj.dataUrl;

                },
                _clearAjaxComboBox: function (self, layer, args) {
                    var obj = args[1];
                    if (obj.fieldHtmlId != self.option.fieldHtmlId) {
                        return;
                    }

                    self._delButtonClick(self);
                },

                _checkGroupMode: function () {

                    this.option.groupMode = !this.option.groupMode;

                    if ($(this.elem.button).attr('title') != this.message.get_all_btn) {
                        this.elem.button.mouseup();
                    }

                    if (this.option.groupMode) {
                        this.elem.enableGroupButton.addClass("ac_button_selected");
                        var source = "/share/proxy/alfresco/picker/group";

                        if (this.option.enableRole && this.option.documentNodeRef) {
                            source += "/negotiationRoles/" + this.option.documentNodeRef.replace("workspace://SpacesStore/", "")
                        }

                        this.option.source = source;
                    } else {
                        this.elem.enableGroupButton.removeClass("ac_button_selected");
                        this.option.source = this.option.originalSource;
                    }
                ...
                _delButtonClick: function (self) {
                    var elem = self.elem;

                    $(elem.combo_input).val('');
                    $(elem.combo_input)[0].removeAttribute("sub_info");
                    $(elem.combo_input).change();
                    var c_id = $(elem.combo_input)[0].id;
                    var val_id = "#" + c_id.substring(0, c_id.length - 4);

                    $(val_id + "-added").val('');
                    $(val_id + "-removed").val($(val_id).val());
                    $(val_id).val('');

                    self._fireChange(self, null);
                },
                ...
                _setElem: function (combo_input) {
                    ...
                    elem.container.css('border-color', $(elem.combo_input).css("border-color"));

                    $(elem.combo_input).css('border', 'none');

                    elem.clear = $('<div>').css('clear', 'left');
                    if (this.option.plugin_type == 'combobox') {
                        elem.buttonContainer = $('<div>').addClass('ac_BtnContainer');
                        elem.button = $('<div>').addClass(this.css_class.button);
                        elem.img = $('<img>').attr('src', this.option.button_img);
                        elem.delbutton = $('<div>').addClass('ac_delBtn');

                        var _this = this;
                        elem.delbutton.click(function () {
                            _this._delButtonClick(_this);
                        });


                        if (this.option.enableGroup || this.option.enableRole) {
                            elem.enableGroupButton = $('<div>').addClass(this.css_class.button);
                            elem.enableGroupButtonImg = $('<img>').attr('src', this.option.group_img);
                            elem.enableGroupButton.append(elem.enableGroupButtonImg);

                            elem.enableGroupButton.click(function () {
                                _this._checkGroupMode();
                            });
                        }
                    ...
                    if (!this.option.hasAreaMaxHeight) {
                        elem.result_area.css('max-height', 'none');
                    }
                    ...
                    if (this.option.plugin_type == 'combobox') {
                        elem.container.width(elem.combo_input.width() + 70);
                    } else if (this.option.plugin_type == 'textarea') {
                        elem.hidden = false;
                    } else {
                    ...
                    switch (this.option.plugin_type) {
                        case 'combobox':
                            $(elem.container)
                                .append(elem.buttonContainer)
                                .append(elem.clear)
                                .append(elem.result_area)
                                .append(elem.hidden);

                            if (this.option.enableGroup || this.option.enableRole) {
                                elem.combo_input.width(elem.combo_input.width() - 30);
                                $(elem.buttonContainer).append(elem.enableGroupButton);
                                elem.enableGroupButton.append();
                            }

                            $(elem.buttonContainer)
                                .append(elem.delbutton)
                                .append(elem.button);


                            $(elem.button).append(elem.img);
                            $(elem.delbutton).append("x");
                            break;
                    ...
                    if (!this.option.withButton) {
                        elem.button.hide();
                        elem.delbutton.css('padding-right', '5px');
                        elem.combo_input.css('width', (Number(elem.combo_input.css('width').replace('px', '')) + 30) + 'px');
                    }
                    ...
                    var resultArea =
                        $(elem.result_area)
                            .append(elem.navi)
                            .append(elem.results);
                    if (this.option.withSubInfoElem) {
                        resultArea.append(elem.sub_info);
                    }
                    $(elem.navi)
                    ...
                    /*
                    if (this.option.plugin_type == 'combobox') {
                        //$(elem.container).width($(elem.combo_input).outerWidth() + 50);
                ...
                _checkValue: function (self) {
                    ...
                    //一定時間ごとの監視を再開
                    if (!this.option.skip_search) self._setTimerCheckValue(self);
                },
                ...
                _suggest: function (self) {
                    ...
                    if (self.option.plugin_type != 'textarea') {
                        ...
                        q_word = encodeURIComponent(q_word.split(/[\s　]+/));
                    } else {
                ...
                _searchForDb: function (self, q_word, which_page_num) {
                    self.prop.xhr = $.getJSON(
                        ...
                        function (json) {
                            ...
                            if (self.option.allInOne) {
                                self.option.per_page = json.cnt_whole;
                            }
                ...
                _setNaviPages: function (self, cnt_whole, cnt_page, page_num) {
                    var num_page_top = self.option.per_page * (page_num - 1) + 1;
                    var num_page_end = num_page_top + cnt_page - 1;
                    var cnt_result = cnt_whole == 0 && self.option.changeNoResultText
                        ? self.option.noResultText
                ...
                _setNavi: function (self, cnt_whole, cnt_page, page_num) {
                    self._setNaviPages(self, cnt_whole, cnt_page, page_num);
                    ...
                    if (last_page > 1) {
                        ...
                        if (page_num == 1) {
                            if (!self.option.navi_simple) {
                                $('<span>')
                                    .text(self.option.navi_simpleFirstLast ? '<<' : '<< 1')
                                    .addClass('page_end')
                                    .appendTo(self.elem.navi_p);
                            }
                            if (self.option.navi_prevNext) {
                                $('<span>')
                        ...
                        if (page_num == last_page) {
                            ...
                            if (!self.option.navi_simple) {
                                $('<span>')
                                    .text(self.option.navi_simpleFirstLast ? '>>' : (last_page + ' >>'))
                                    .addClass('page_end')
                                    .appendTo(self.elem.navi_p);
                            }
                        } else {
				...
				arr_primary_key[i] == $(self.elem.hidden).val()*/
                            ...
                            $list.addClass(i % 2 != 0 ? self.css_class.selected : (self.css_class.selected + "_even"));
                        }
                        ...
                        if (arr_subinfo) {
                            ...
                            for (key in arr_subinfo[i]) {
                                ...
                                if (arr_subinfo[i][key] == null) {
                                    ...
                                    // DBのデータ値が数値の場合の対処
                                    if (typeof arr_subinfo[i][key] == "object" || Array.isArray(typeof arr_subinfo[i][key])) {
                                        arr_subinfo[i][key] = JSON.stringify(arr_subinfo[i][key]);
                                    } else {
                                        arr_subinfo[i][key] += '';
                                    }
                                }
                ...
                _calcWidthResults: function (self) {
                    ...
                    if (self.option.plugin_type == 'combobox') {
                        var w = $(self.elem.combo_input).outerWidth() + $(self.elem.button).outerWidth() + 1;
                    } else {
                ...
                _selectCurrentLine: function (self, is_enter_key) {
                    ...
                    if (current) {
                        ...
                        si = $(current).attr('sub_info').toString();
                        if (si) {
                            var siJson = eval('(' + si + ')');

                            if (siJson.values) {
                                var values = eval('(' + siJson.values + ')');
                                values.forEach(function (value) {

                                    self._selectCurrent(self, is_enter_key,
                                        {
                                            text: value.name,
                                            sub_info: JSON.stringify(value),
                                            pkey: $(current).attr('pkey')
                                        });
                        ...
                        self._selectCurrent(self, is_enter_key,
                            {
                                text: $(current).text(),
                                sub_info: $(current).attr('sub_info'),
                                pkey: $(current).attr('pkey')
                            });
                ...
                _selectCurrent: function (self, is_enter_key, data) {
                    if (self.option.plugin_type != 'textarea') {
                        $(self.elem.combo_input).val(data.text);

                        // サブ情報があるならsub_info属性を追加・書き換え
                        if (self.option.sub_info) {
                            $(self.elem.combo_input).attr('sub_info', data.sub_info);
                        }
                        if (self.option.select_only) self._setButtonAttrDefault();
                        $(self.elem.hidden).val(data.pkey);
                    } else {
                    ...
                    $(self.elem.combo_input).change(); // テキストボックスの値が変わったことを通知
                    self._setCssFocusedInput(self);
                    self._fireChange(self, data);


                },

                _fireChange: function (self, data) {

                    if (self.option.fireAction) {

                        YAHOO.Bubbling.fire(self.option.fireAction, {
                            obj: data,
                            fieldHtmlId: self.option.fieldHtmlId,
                            formId: self.option.formId
                        });
