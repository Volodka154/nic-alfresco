...
function generateAppItems() {
    ...
    if (user.isAdmin || user.properties["alfUserGroups"].indexOf("GROUP_Пользователи СЭД") !== -1) {
        appItems.push({
            id: "HEADER_DOCLIST",
            name: "alfresco/menus/AlfMenuBarItem",
            config: {
                id: "HEADER_DOCLIST",
                label: "Реестры",
                targetUrl: "user/" + encodeURIComponent(user.name) + "/search-registry"
    ...
    if (user.isAdmin || user.properties["alfUserGroups"].indexOf("GROUP_BTLab_ReplacementEditor") !== -1 || user.properties["alfUserGroups"].indexOf("GROUP_BTLab_ContactorContactPersonEditor") !== -1) {
        appItems.push({
            id: "HEADER_ADMIN_CONSOLE_BTL",
            name: "alfresco/menus/AlfMenuBarItem",
            config: {
                id: "HEADER_ADMIN_CONSOLE_BTL",
                label: "Справочники",
                targetUrl: "dp/ws/dictionaries"
                //targetUrlLocation: "NEW",
            }
        });
    }
...
function getUserMenuWidgets() {
    var userMenuWidgets = [
        getUserStatusWidget(),
        {
            id: "HEADER_USER_MENU_SET_DELEGATES",
            name: "alfresco/header/AlfMenuItem",
            config: {
                id: "HEADER_USER_MENU_SET_DELEGATES",
                label: "Настроить замещение",
                iconClass: "alf-user-profile-icon",
                publishTopic: "ALF_SET_USER_DELEGATES"
            }
        },
        {
                        ...
                        [
                            ...
                            },
                            {
                                ...
                                config:
                                    {
                                        id: "HEADER_USER_MENU_SET_DASHBOARD_AS_HOME",
                                        label: "Использовать домашнюю страницу",
...
function getHeaderServices() {
    var services = [
        ...
        "meeting/forms/SelectRooms",
        "isid-share-jar/js/services/ChangeDelegateService",
