...
(function()
{
   ...
   Alfresco.WebPreview.prototype.Plugins.PdfJs.prototype =
   {
      ...
      onViewerLoaded: function PdfJs_onViewerLoaded(p_obj)
      {
		 ...
		 this.widgets.turnButton = Alfresco.util.createYUIButton(this, "turn", this.onTurn, {
            disabled: false
         }, this.wp.id + "-turn");
         ...
         this.widgets.scaleMenu.getMenu().subscribe("click", this.onZoomChange, null, this);
          var downloadMenu = [
			...
			{ text: "Печать", value: "", onclick: { fn: this.print, scope: this } },
         ];
		 
         if (this.attributes.src)
         {
            downloadMenu.push(
							{ text: this.wp.msg("link.downloadPdf"), value: "", onclick: { fn: this.onDownloadPDFClick, scope: this } }							
	  ...
	  print: function PdfJs__print()
	   {
		    var fileurl = this.attributes.src ? this.wp.getThumbnailUrl(this.attributes.src) : this.wp.getContentUrl();
			document.WinPrint = window.open(fileurl,'','left=50,top=50,width=800,height=640,toolbar=0,scrollbars=1,status=0');			
			document.WinPrint.focus();
			document.WinPrint.print();		
      ...
      _setPreviewerElementHeight: function PdfJs_setPreviewerElementHeight()
      {
         ...
         if (!this.maximized)
         {
            ...
            else
            {
               ...
               Dom.setStyle(this.wp.getPreviewerElement(), "height", "98vh");
            }
         }
	  ...
	  onTurn : function PdfJs_onTurn(e_obj)
      {
		if (this.documentView.pages[0].container.style.transform == "rotate(90deg)")
		{
			this.turnPages("rotate(180deg)");
		}
		else if (this.documentView.pages[0].container.style.transform == "rotate(180deg)")
		{
			this.turnPages("rotate(-90deg)");
		}
		else if (this.documentView.pages[0].container.style.transform == "rotate(-90deg)")
		{
			this.turnPages("none");
		}
		else
	  ...
	  turnPages: function PdfJs_turnPages(rotate)
	  {
		  for (var i=0; i< this.documentView.pages.length; i++ )
		  {
			  this.documentView.pages[i].container.style.transform = rotate;			  
		  }		 		  
	  },
