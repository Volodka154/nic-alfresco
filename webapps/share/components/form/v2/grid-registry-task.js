...
/***
 * Запрос групп для текущего пользователя с последующитм создание таблицы "Задачи"
 */
function showTask() {
 ...
 * @param isArch
 */
function creategridTask(isArch, isAdmin) {
    var pq = getDefaultPq();
    ...
    var stateAccess = [
        ...
        {"Делегированные (на проверке)": "Делегированные (на проверке)"},
        {"Активные. Родительская завершена": "Активные. Родительская завершена"},
        {"Активные без исполнителя": "Активные без исполнителя"},
        {"Завершенные без исполнителя": "Завершенные без исполнителя"},
        {"Разные исполнители": "Разные исполнители"},
        {"Просроченные исполненные": "Просроченные исполненные"},
        {"Просроченные неисполненные": "Просроченные неисполненные"}
    ];
    ...
    var employeeForTask = document.emploeeDoc.slice(0);
    employeeForTask.unshift({"Уволенные сотрудники": "Уволенные сотрудники"});
    ...
    var colModel = [
        ...
        },
        {
            ...
            width: 127,
            dataType: "string",
            dataIndx: "btl-task:author-login",
            filter: {
                type: 'select',
                style: "height:32px;",
                condition: 'equal',
                options: employeeForTask,
                prepend: {'': ''},
                listeners: ["change"],
                init: function () {
                    $(this).pqSelect({
        ...
        },
        {
            ...
            width: 127,
            dataType: "string",
            dataIndx: "btl-task:executor-login",
            filter: {
                type: 'select',
                style: "height:32px;",
                condition: 'equal',
        ...
        },
        {
            ...
            width: 127,
            dataType: "string",
            dataIndx: "btl-task:actualExecutor-login",
            filter: {
                type: 'select',
                style: "height:32px;",
                condition: 'equal',
        ...
        },
        {
            ...
            dataType: "string",
            hidden: false,
            dataIndx: "btl-task:parentDocument-content",
            filter: {
                type: 'select',
                condition: 'contain',
                options: [],
                ...
                init: function (arg1) {
                    $(this).pqSelect({
                        ...
                        searchRule: "begin",
                        width: '100%',
                        url: "/share/proxy/alfresco/filters/association?type=btl-document:documentDataType&s_field=btl-document:reg-number&field=btl-document:reg-number",
                        urlItem: "/share/proxy/alfresco/picker/associationItem?type=btl-document:documentDataType&field=btl-document:reg-number",
        ...
        },
        {
            ...
            width: 100,
            dataType: "string",
            dataIndx: "btl-task:delegates-content",
            sortable: false
        }
    ];
    ...
    if (isArch) {
        colModel.unshift({
            title: "<input type='checkbox' id='selAllTask' onclick='selectAllInGrid(this, $gridTask)'>",
            dataIndx: "sel",
            width: 30,
            align: "center",
    ...
    var dataModel = {
        ...
        getUrl: function (ui) {
            if ($(this).pqGrid("option", "isInit2") !== true) {
                ...
                $(this).pqGrid("option", "currentCountId", "f2TaskCurrentCount");
                $(this).pqGrid("option", "totalCountId", "f2TaskTotalCount");
                $(this).pqGrid("option", "dataUrl", "/share/proxy/alfresco/documents/v2/search-registry-task");
    ...
    if (isAdmin) {
        newObj.title += "<button type='button'  style='margin-left: 5px'  onclick='deleteSelectTask()' class='ui-corner-all  ui-button ui-widget ui-state-default ui-button-text-icon-primary'" +
            " role='button'><span class='ui-button-icon-primary ui-icon ui-icon-cancel'></span><span class='ui-button-text'>Удалить выбранные</span></button>";
    }
    newObj.title += "<button type='button'  style='margin-left: 5px' onclick='prnTasks()' class='ui-corner-all  ui-button ui-widget ui-state-default ui-button-text-icon-primary'" +
        " role='button'><span class='ui-button-icon-primary ui-icon ui-icon-print'></span><span class='ui-button-text'>Печать</span></button>"+
        "<button type='button'  style='margin-left: 5px'  onclick='clearGridFilters($gridTask)' class='ui-corner-all  ui-button ui-widget ui-state-default ui-button-text-icon-primary'" +
        " role='button'><span class='ui-button-icon-primary ui-icon ui-icon-cancel'></span><span class='ui-button-text'>Очистить</span></button>" +
        "<button type='button' style='margin-left: 5px' onclick='refreshDataAndView($gridTask)' class='ui-corner-all  ui-button ui-widget ui-state-default ui-button-text-only' role='button'><span class='ui-button-text'>Обновить</span></button>";
...
function openModalTask(nodeRef) {
    document.frameSave = function () {
        ...
        $gridTask.pqGrid("option", "isResetData", true);
...
function getArgsAndShowForm(nodeRef) {
    ...
    var ajaxesPars3 = [
        {
            url: "document/get-next-consideration-document",
            args: {nodeRef: nodeRef},
            varName: "nextDoc",
            requires: {docStatus: "На рассмотрении", isGD: "true", isEEO: "true"}
        },
        {
            url: "document/get-document-gd-setting",
            args: {nodeRef: nodeRef},
            varName: "gdSetting",
            requires: {docStatus: "На исполнении", isGD: "true"}
        }
    ];
            ...
            if (ajaxResult && (res = ajaxResult[0])) {
                switch (ajaxPars.varName) {
                    ...
                    case "gdSetting": {
                        ...
                        break;
                    default:
                        resultPars[ajaxPars.varName] = getArgFromCheckResult(ajaxResult, ajaxPars);
                        break;
                }
            }
        }
