    ...
    YAHOO.extend(Alfresco.FormUI, Alfresco.component.Base,
        {
            ...
            deleteButtonClick: function()
            {
                ...
                var fnFormLoaded = function(response)
                {
                ...
                var data =
                {
                    cardId: this.options.arguments.itemId
                };


                var dialog = new YAHOO.widget.SimpleDialog("delete_dialog_block_window", {
                    width: "100%",
                    height: "100%",
                    x: 0,
                    y: 0,
                ...
                Alfresco.util.Ajax.request(
                    {
                        url: "/share/proxy/alfresco/delete",
                        dataObj: data,
                        successCallback: {
                            fn: fnFormLoaded,
                            scope: this
            ...
            onButtonSubmitClick: function onButtonSubmitClick(){
                this.options.arguments.forceRedirect = true;
            },
            onButtonSaveClick: function onButtonSaveClick(){
                this.options.arguments.forceRedirect = false;
            },

            onButtonPopUpClick: function onButtonPopUpClick () {
                this.isButtonPopUpClick = true;
            },
            ...
            onReady: function FormUI_onReady()
            {
                if (this.options.mode !== "view")
                {
					...
					if (this.buttons.submit)
						this.buttons.submit.on("click", this.onButtonSubmitClick.bind(this));

                    if (Dom.get(this.id + "-save") !== null)
                    {
                        this.buttons.save = Alfresco.util.createYUIButton(this, "save", null,
                            {
                                type: "submit"
                            });

                        // force the generated button to have a name of "-" so it gets ignored in
                        Dom.get(this.id + "-save-button").name = "-";
                    }
					if (this.buttons.save)
						this.buttons.save.on("click", this.onButtonSaveClick.bind(this));

                    if (Dom.get(this.id + "-savePopUpButton") !== null)
                    {
                        this.buttons.savePopUpButton = Alfresco.util.createYUIButton(this, "savePopUpButton", null,
                            {
                                type: "submit"
                            });

                        // force the generated button to have a name of "-" so it gets ignored in
                        Dom.get(this.id + "-savePopUpButton-button").name = "-";
                    }
                    if (this.buttons.savePopUpButton)
                        this.buttons.savePopUpButton.on("click", this.onButtonPopUpClick.bind(this));
                    ...
                    if (Dom.get(this.id + "-delete") !== null)
                    {
                        this.buttons.buttonDelete = Alfresco.util.createYUIButton(this, "delete", this.deleteButtonClick);

                        // force the generated button to have a name of "-" so it gets ignored in
                        // JSON submit. TODO: remove this when JSON submit behaviour is configurable
                        Dom.get(this.id + "-delete-button").name = "-";
                    }
            ...
            onJsonPostSuccess: function FormUI_onJsonPostSuccess(response)
            {
                ...
                responseText = JSON.parse(response.serverResponse.responseText);

                if(this.isButtonPopUpClick){

                    document.getElementById('buttonCancel').click();
                    if (this.options.arguments.fillField != null && this.options.arguments.fillField.length > 0)
                    {

                        var fnAssociationLoaded = function(response, p_formUI)
                        {
                            var n = document.getElementById(this.options.arguments.fillField + "-val");
                            n.value = eval( '(' + response.serverResponse.responseText + ')' ).item;
                        ...
                        Alfresco.util.Ajax.request(
                            {
                                url: "/share/proxy/alfresco/picker/associationItem?nodeRef="+ responseText.persistedObject + this.options.arguments.fillFieldparams,
                                successCallback: {
                                    fn: fnAssociationLoaded,
                                    scope: this
                                },
                                scope:this,
                                execScripts:true
                        ...
                        var n = document.getElementById(this.options.arguments.fillField);
                        if (n)
                        	n.value = responseText.persistedObject;

                        n = document.getElementById(this.options.arguments.fillField + "-added");
                        if (n)
                        	n.value = responseText.persistedObject;
                ...
                if (this.options.arguments.forceRedirect == true)
                {
                    if (this.options.arguments.redirect.length > 0)
                        window.location.assign(this.options.arguments.redirect);
                    else
                	{
                ...
                if (this.options.arguments.openEditForm == true)
                {
                    if (this.options.arguments.openModalForm == true)
                    {
                        var modalEditDiv = Dom.get("modalWindow");

                        if (modalEditDiv)
                        {
                            var fnFormLoaded = function(response, p_formUI)
                            {
                                modalEditDiv.innerHTML = response.serverResponse.responseText;
                            };

                            var data =
                            {
                                htmlid: "modalEditDiv_body",
                                itemKind: "node",
                                itemId: responseText.persistedObject,
                                mode: "edit",
                                submitType: "multipart",
                                showCaption: true,
                                showCancelButton: false,
                                formUI: true,
                                redirect:this.options.arguments.redirect,
                                destination: ""
                            };
                    ...
                    if (this.options.arguments.forceRedirect == true){
                        ...
                        window.location.assign("/share/page/btl-edit-metadata?nodeRef=" + responseText.persistedObject + "&redirect=" + this.options.arguments.redirect);
                    }
