...
(function()
{
   ...
   Alfresco.DatePicker = function(htmlId, currentValueHtmlId)
   {
      ...
      this.onComponentsLoaded.call(this);
   ...
   Alfresco.DatePicker.prototype =
       {
          ...
          onReady: function DatePicker_onReady()
          {
             ...
             if (this.options.minToday)
             {
                this.widgets.calendar.cfg.minToday = this.options.minToday;
             }
          ...
          {
              var endTimeInput = $('#'+this.id+'-date');
              var endTimeInputPos = endTimeInput.position();
              $('#'+this.id).css({top:endTimeInputPos.top+endTimeInput.height()+14,left:endTimeInputPos.left-20})
