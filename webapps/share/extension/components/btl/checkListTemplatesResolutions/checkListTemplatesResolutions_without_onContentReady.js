(function () {
    ...
    Alfresco.CheckListTemplatesResolutions = function BTL_CheckListTemplatesResolutions(htmlId) {
        // Alfresco.CheckListTemplatesResolutions.superclass.constructor.call(this, "Alfresco.CheckListTemplatesResolutions", htmlId, []);
        this.name = "Alfresco.CheckListTemplatesResolutions";
        this.id = htmlId;
    ...
    Alfresco.CheckListTemplatesResolutions.prototype = {
        options: {
            url: "/share/proxy/alfresco/search/taskResolution",
            dataUrl: {
                type: "0",
                matchByRank: true
            },
        ...
        setOptions: function CheckListTemplatesResolutions_setOptions(obj) {
            if ('dataUrl' in obj) {
                this.options.dataUrl = YAHOO.lang.merge(this.options.dataUrl, obj.dataUrl);
                delete obj.dataUrl;
            }
            // Alfresco.CheckListTemplatesResolutions.superclass.setOptions.call(this, obj);

            return this;
        },
        onReady: function CheckListTemplatesResolutions_onReady() {
            this.formCreate();
            this.loadData();
            //this.init();
        },
        formCreate: function formCreate() {
            ...
            fioCell.innerHTML = "Шаблоны резолюций";
        ...
        loadData: function CheckListDeadlines_loadData() {
            ...
            var onSuccess = function CheckList_load_success(response) {
                ...
                if(items.length>0){
                    var title = createElementFromHTML("<span><b>Резолюции<b></b></b></span>");
        ...
        getCheckedName: function getCheckedName() {
            var values = [];
            var elements = this.tableMainBody.querySelectorAll(".checked");
            for (var i = 0; i < elements.length; i++) {
                values.push(elements[i].cells[1].innerHTML);
            }
            return values.join(", ");
        },
