...
gantt = {version: "4.0.0"}, gantt.event = function (t, e, n) {
    ...
    if (t.getBoundingClientRect) {
        var a = t.getBoundingClientRect(), i = document.body, s = document.documentElement,
            r = window.pageYOffset || s.scrollTop || i.scrollTop,
            o = window.pageXOffset || s.scrollLeft || i.scrollLeft, _ = s.clientTop || i.clientTop || 0,
    ...
    t = t || window.event;
    var e = t.target || t.srcElement, n = gantt.locate(e), a = gantt.locate(e, gantt.config.link_attribute),
    ...
    dragStart: function (t, e) {
        ...
        if (!this.config.marker && !this.config.started) {
            var n = this.getPosition(e), a = n.x - this.config.pos.x, i = n.y - this.config.pos.y,
    ...
    for (var t = this.getGridColumns(), e = [], n = 0, a = this.locale.labels, i = this.config.scale_height - 2, s = 0; s < t.length; s++) {
        ...
        var d = this._sort && o.name == this._sort.name ? "<div class='gantt_sort gantt_" + this._sort.direction + "'></div>" : "",
            ...
            g = "width:" + (_ - (r ? 1 : 0)) + "px;", h = o.label || a["column_" + o.name];
        h = h || "";
    ...
    gantt._render_grid_item = function (t) {
        ...
        for (var e = this.getGridColumns(), n = [], a = 0; a < e.length; a++) {
            ...
            change = '';
            if (o.change) {
                change = 'projectcheckDateDuration(\'' + t.id + '\',\'' + t.parent + '\')';
            }

            if (o.checkEndDate) {
                change = 'projectcheckEndDate(\'' + t.id + '\',\'' + t.parent + '\')';
            }

            dateClick = false;
            if (t.index > 1) {
                dateClick = false;
                if (o.name == 'start_date') {
                    field = 'prop_btl-task_startDate';
                    dateClick = true;
                } else if (o.name == 'end_date') {
                    field = 'prop_btl-task_endDate';
                    dateClick = true;
                }

                if (dateClick) {

                    var f = new Date(titleName);
                    var dd = f.getDate();
                    var m = f.getMonth() + 1;
                    var y = f.getFullYear();

                    var dateValue = addZero(dd) + "." + addZero(m) + "." + y;

                    if (o.edit){
                        if (o.change) {
                            change = 'projectcheckDateDuration(\'' + t.id + '\',\'' + t.parent + '\'); document.getElementById(\'' + t.id + '_isAutoDate\').innerHTML = \'\'; ';
                        }

                        var strclass = "ganttTableInputDate";

                        var CurValueControl = document.getElementById(t.id + '-' + o.name + '-value');
                        if (CurValueControl != null) {
                            controlValue = CurValueControl.value;

                        }


                        datetext = '<input name="' + t.id + '-' + o.name + '-value" id="' + t.id + '-' + o.name + '-value" type="text" class="' + strclass + '" value="' + dateValue + '" >';

                        datetext += '<a tabindex="0" id="' + t.id + '-' + o.name + '-endTime-icon" onclick="showCalendar(\'' + t.id + '\', \'' + o.name + '\')" style="float: right;">';
                        datetext += '<img tabindex="0" class="datepicker-icon" src="/share/res/components/form/images/calendar.png" style="height: 80%; vertical-align: -4px;">';
                        datetext += '</a>';
                        datetext += '<div id="' + t.id + '-' + o.name + '-calendar" class="datepicker yui-calcontainer single withtitle"></div>';
                    }
                    else {
                        datetext = dateValue;
                    }
            ...
            "add" == o.name ? s = "<div class='gantt_add'></div>" : (s = o.template ? o.template(t) : t[o.name], s instanceof Date && (s = datetext), s = "<div class='gantt_tree_content'>" + s + "</div>");
            var _ = "gantt_cell" + (r ? " gantt_last_cell" : ""), d = "";

            if (o.employee && t.index > 1) {
                employeeName = t[o.name];
                var employeeValue = document.getElementById(t.id + "-employee");
                if (employeeValue) {
                    employeeName = employeeValue.innerHTML;
                }

                if (o.edit){
                    employeeText = '<span class="editdateButton" onclick="editEmployee(\'' + employeeName + '\',\'' + t.id + '\')" style="float:right">&#9998;</span>';
                    s = "<div class='gantt_tree_content' style='width:100%;'><span id='" + t.id + "-employee' style='float:left'>" + employeeName + "</span>" + employeeText + "</div>"
                } else {
            ...
            if (o.edit == true) {
                if (t["inputValue" + o.name] == null) {
                    t["inputValue" + o.name] = t[o.name];
                }
                else {
                    var inputval = document.getElementById(t.id + "_" + o.name);
                    if (inputval)
                        t["inputValue" + o.name] = inputval.value;
                    else
                        t["inputValue" + o.name] = t[o.name];
                }
                ...
                var inputValue = t["inputValue" + o.name];

                st = inputValue.length > 25 ? 'style="overflow: hidden; text-overflow:ellipsis; white-space: nowrap; width: 17em !important;"' : "";
                s = '<input id="' + t.id + '_' + o.name + '" ' + type + '  value="' + t["inputValue" + o.name] + '" class="' + o.style + '"' + st + ' onchange="' + change + '" >';
            }
            ...
            if (o.tree) {
                ...
                //d+=this.templates.grid_blank(t),d+=this.templates.grid_file(t);
                g ? (d += this.templates.grid_open(t)) : (d += this.templates.grid_blank(t))

                var num = "";

                if (o.num) {
                    num = "<div class='gantt_cell' style='width: 35px;'>" + t.index + "</div>";
                }

                var h = "width:" + (o.width - (r ? 1 : 0) - 35) + "px;";
                this.defined(o.align) && (h += "text-align:" + o.align + ";"), i = num + "<div class='" + _ + "' style='" + h + "' title='" + titleName + "'>" + d + s + "</div>", n.push(i)

            }
            else {
                var h = "width:" + (o.width - (r ? 1 : 0)) + "px;";
                this.defined(o.align) && (h += "text-align:" + o.align + ";"), i = "<div class='" + _ + "' style='" + h + "'>" + d + s + "</div>", n.push(i)
            }
            ...
            if (dateClick) {
                ...
                t[o.name].Value = getStringDate(t[o.name]);
                t[o.name].controlValue = getStringDate(val);
            }
        ...
        u.className = "gantt_row" + _,
            u.style.height = rowScale * this.config.row_height + "px",
            u.style.lineHeight = rowScale * gantt.config.row_height + "px",
            u.setAttribute(this.config.task_attribute, t.id), u.innerHTML = n.join(""), u;
    ...
    if (t != n) {
        ...
        var i = this.getTask(t), s = this.getParent(i.id),
    ...
    this.defined(this.config.dnd_sensitivity) && (t.config.sensitivity = this.config.dnd_sensitivity),
        ...
        var r = (s.x, s.y), o = document.documentElement.scrollTop || document.body.scrollTop,
            _ = document.documentElement.scrollLeft || document.body.scrollLeft,
            d = document.elementFromPoint(s.x - _ + 1, r - o), l = this.locate(d), g = this.getTask(t.config.id);
    ...
    var e = this._path_builder.get_points(t), n = gantt._drawer, a = n.get_lines(e), i = document.createElement("div"),
        s = "gantt_task_link";
    ...
    var d = a[a.length - 1].direction,
        l = gantt._render_link_arrow(e[e.length - 1], d, t.type);

    return t.color && (l.style.borderColor = t.color), i.appendChild(l), i
},
    gantt._render_link_arrow = function (t, e, type) {
        var n = document.createElement("div"),
            a = gantt._drawer, i = t.y, s = t.x,
            r = gantt.config.link_arrow_size,
            o = gantt.config.row_height, _ = "gantt_link_arrow gantt_link_arrow_" + e;
        var typeColor = "";
        if (type == "0")
            typeColor = "border-color: #0046c6;";
        ...
        return n.style.cssText = ["top:" + i + "px", "left:" + s + "px", typeColor].join(";"), n.className = _, n
    }, gantt._drawer = {
    current_pos: null,
    ...
    get_line_sizes: function (t) {
        ...
        var dopstyle = "";
        if (t.type == "0")
            dopstyle = "background-color: #0046c6;"
        var a = this.get_line_sizes(t), i = document.createElement("div");
        return i.style.cssText = ["height:" + a.height + "px", "width:" + a.width + "px", "margin-top:" + a.marginTop + "px", "margin-left:" + a.marginLeft + "px", dopstyle].join(";"), i.className = "gantt_link_line_" + t.direction, n.appendChild(i), n
    },
    ...
    function s(t, e, n, i) {
        ...
        if (g = Math.max(0, g - 3)) {
            ...
            var u = Math.sin(c), f = Math.cos(c), p = Math.round(e), v = Math.round(t),
    ...
    var l = new gantt._DnD(this.$task_bars, {sensitivity: 0, updates_per_second: 60}), g = "task_left",
        h = "task_right", c = "gantt_link_point", u = "gantt_link_control";
    l.attachEvent("onBeforeDragStart", gantt.bind(function (e, n) {
        ...
        n(r.marker, o);
        var _ = gantt._is_link_drop_area(i), d = gantt._link_target_task, g = gantt._link_landing,
            c = gantt._link_target_task_start, f = gantt.locate(i), p = !0;
    ...
    n && n.setAttribute(this.config.layer_attribute, !0), this._task_area_renderers[t] = {
        render_item: function (t, i) {
            ...
            this.clear(e);
            if(e.getAttribute("class")=="gantt_grid_data"){
                gantt.config.projectHasChild = Boolean(t.length!=1);
            }
    ...
    this._tasks = {
        ...
        var n = gantt._get_mouse_pos(t), a = gantt.dateFromPos(n.x), i = Math.floor(gantt._day_index_by_date(a)),
    ...
    var _ = t.$index % 2 !== 0, d = gantt.templates.task_row_class(t.start_date, t.end_date, t),
    ...
    if (r.link_source_id == n && a.push("gantt_link_source"), r.link_target_id == n && a.push("gantt_link_target"), this.config.highlight_critical_path && this.isCriticalTask && this.isCriticalTask(s) && a.push("gantt_critical_task"), r.link_landing_area && r.link_target_id && r.link_source_id && r.link_target_id != r.link_source_id) {
        var o = r.link_source_id, _ = r.link_source_start, d = r.link_target_start, l = gantt.isLinkAllowed(o, n, _, d),
            g = "";
    ...
    if (!this._isAllowedUnscheduledTask(t)) {
        var e = this._get_task_pos(t), n = this.config, a = this._get_task_height(),
...
gantt.removeLinkLayer = function (t) {
...
},gantt.getEditTask = function () {
    editTasks = [];

    for (var i = 0; i < this._order_full.length; i++) {

        var task = this._pull[this._order_full[i]];
        if (task.parent != '') {
            var startDate = document.getElementById(task.id + "-start_date-value").value;
            var value = document.getElementById(task.id + "_text").value;
            var dur = document.getElementById(task.id + "_dur").value;
            var endDate = document.getElementById(task.id + "-end_date-value").value;
            var isAutoDate = document.getElementById(task.id + "_isAutoDate").innerHTML;

            if (isAutoDate.trim().length > 0)
                isAutoDate = true;
            else
                isAutoDate = false;
            ...
            if ((task.isAutoDate != "true" && isAutoDate == true) || (task.isAutoDate == "true" && isAutoDate == false))
                isNotAuto = true


            if (value != task.text || (task["start_date"].Value != startDate) || (task["end_date"].Value != endDate) || dur != task.dur || isNotAuto) {
                editTasks.push({
                    "id": task.id,
                    "value": value,
...
},gantt.getTaskById = function (p) {
    return this._pull[p];

},gantt.getTaskByNumber = function (p) {
    for (var i = 0; i < this._order_full.length; i++) {
        var task = this._pull[this._order_full[i]];
        if (task.index == p) {
            return task;
...
},gantt.getChildNoPrevious = function (p) {
    childTasks = [];
    for (var i = 0; i < this._order_full.length; i++) {
        var task = this._pull[this._order_full[i]];
        if (task.parent == p) {
    ...
    return childTasks;
},gantt.getPrevious = function (p) {
    previousTasks = [];
    for (var i = 0; i < this._order_full.length; i++) {
        var task = this._pull[this._order_full[i]];
        if (task.previousId == p) {
    ...
    get_closest_worktime: function (t) {
        ...
        var e = t.unit, n = gantt.date[e + "_start"](t.date), a = new Date(n), i = new Date(n), s = !0, r = 3e3, o = 0,
            _ = "any" == t.dir || !t.dir, d = 1;
...
gantt._lightbox_template = "<div class='gantt_cal_ltitle'><span class='gantt_mark'>&nbsp;</span><span class='gantt_time'></span><span class='gantt_title'></span></div><div class='gantt_cal_larea'></div>",gantt.showLightbox = function (t) {
    ...
    for (var i = this._get_typed_lightbox_config(this.getLightboxType()), s = 0; s < i.length; s++) {
        ...
        if (this.form_blocks[r.type]) {
            var o = document.getElementById(r.id).nextSibling, _ = this.form_blocks[r.type],
                d = gantt._resolve_default_mapping(i[s]), l = this.defined(n[d]) ? n[d] : r.default_value;
    ...
    var i = document.getElementById(a.id), s = i.nextSibling, r = {
        section: a, header: i, node: s, getValue: function (t) {
    ...
    getTimePicker: function (t, e) {
        ...
        render: function (t) {
            ...
            var n = this.locale.labels[this.config.duration_unit + "s"],
                a = t.single_date ? ' style="display:none"' : "", i = t.readonly ? " disabled='disabled'" : "",
            ...
            function i() {
                var e = gantt.form_blocks.duration._get_start_date.call(gantt, t, a),
                    n = gantt.form_blocks.duration._get_duration.call(gantt, t, a), i = gantt.calculateEndDate(e, n);
            ...
            var r = a, o = t.getElementsByTagName("select"), _ = t.getElementsByTagName("input"), d = _[1],
                l = [_[0], _[2]], g = t.getElementsByTagName("span")[0], h = a._time_format_order;
            ...
            var u = n[c.start_date] || new Date, f = n[c.end_date] || gantt.calculateEndDate(u, 1),
            ...
            var a = gantt.form_blocks.duration._get_start_date(t, n),
                i = gantt.form_blocks.duration._get_duration(t, n), s = gantt.calculateEndDate(a, i),
    ...
    var e = t, n = {
        render: e.render, focus: e.focus, set_value: function (t, a, i, s) {
            ...
            if (gantt.callEvent("onSectionToggle", [gantt._lightbox_id, e]), t.style.display = e.disabled ? "none" : "block", e.button) {
                var n = t.previousSibling.firstChild.firstChild, a = gantt.locale.labels,
    ...
    setTransactionMode: function (t, e) {
        ...
        if (window.JSON) try {
            var a = JSON.parse(e.xmlDoc.responseText), i = a.action || this.getState(n) || "updated", s = a.sid || n[0],
                r = a.tid || n[0];
        ...
        var t = this, e = this.obj.getUserData(0, "version"),
    ...
    var t = this.config.scale_height, e = this._order.length * this.config.row_height,
    ...
    var t = this._get_grid_width(), e = Math.max(this._x - t, 0), n = Math.max(this._y - this.config.scale_height, 0),
        a = this.config.scroll_size + 1, i = Math.max(this.$task_data.offsetWidth - a, 0),
        s = this.config.row_height * this._order.length, r = this._get_resize_options(),
        o = this._scroll_hor = r.x ? !1 : i > e, _ = this._scroll_ver = r.y ? !1 : s > n,
        d = {x: !1, y: !1, scroll_size: a, x_inner: i + t + a + 2, y_inner: this.config.scale_height + s};
    ...
    init: function () {
        ...
        var a = n.getFullYear(), i = Math.round((n.getTime() - new Date(a, 0, 1).getTime()) / 864e5),
            s = 1 + Math.floor(i / 7);
        return s
...
function () {
    ...
    function t(t, e) {
        var n = gantt.env.isIE ? "" : "%c",
            ...
            i = window.console.warn || window.console.log, s = [a];
